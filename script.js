...
if (typeof supabase === 'undefined') {
    console.error('Supabase库未加载，等待加载...');
    
    // 如果Supabase未定义，等待一下再初始化
    setTimeout(() => {
        if (typeof supabase === 'undefined') {
            console.error('Supabase库仍然未加载，请检查网络或CDN链接');
            return;
        }
        initApp();
    }, 1000);
...
function initApp() {
    console.log('Supabase库已加载，开始初始化应用...');
                ...
                bookDetailContainer.innerHTML = `
                    <div class="book-detail-header">
                        ...
                        <div class="book-meta">
                            ...
                            <p class="description">${book.description || '暂无描述'}</p>
                            <span class="category">${book.categories?.name || '未知分类'}</span>
                        </div>
                    </div>
                `;
            }
